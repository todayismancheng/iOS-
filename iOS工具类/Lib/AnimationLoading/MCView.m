...
#import "MCView.h"
#import "MCAnimation.h"
#import "MCCenterBGView.h"

@interface MCView ()

//中心背景视图
@property (nonatomic,strong)MCCenterBGView *centerBGView;
 ...
 *  @return 单例对象
 */
+ (MCView *)shareInstanceMCView{
    
    static MCView * instance = nil;
    if (!instance) {
        
        instance                     = [[MCView alloc] initWithFrame:[UIScreen mainScreen].bounds];
        instance.centerBGView        = [[MCCenterBGView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
        /*加载动画现实的中新图标位置*/
        //instance.centerBGView.center = CGPointMake(ISWidth /2,ObjectDetailsHeight/2);
...
+(void)backgroudColor:(UIColor *)color{
    [[self class] shareInstanceMCView].backgroundColor = color;
}
...
+ (void)showLoadingOnView:(UIView *)superView Type:(MCViewType)type{
    /*        在显示前  先从父视图移除当前动画视图         */
    MCView *instance = [[self class] shareInstanceMCView];
    ...
    switch (type) {
        ...
        case MCViewTypeTriangleTranslate:
        {
...
+ (void)showLoadingOnCurrentView:(UIView *)currentView Type:(MCViewType)type{
    ...
    [[self class] showLoadingOnView:currentView Type:type];
}
...
+ (void)hideLoading{
    ...
    [[[self class] shareInstanceMCView] removeFromSuperview];
}
...
- (CALayer *)lineAnimation{
    ...
    [shape addAnimation:[MCAnimation animationForScaleSmall] forKey:nil];
...
+ (CALayer *)qurareAnimation{
    ...
    animationGroup.animations           = @[[MCAnimation animationForScaleAutoreverses], [MCAnimation animationForAlpha]];
...
+ (CALayer *)triangleAnimation{
    ...
    shape.fillColor                    = [UIColor redColor].CGColor;
    shape.lineWidth                    = 1;
    [shape addAnimation:[MCAnimation rotateAnimation] forKey:@"rotateAnimation"];
